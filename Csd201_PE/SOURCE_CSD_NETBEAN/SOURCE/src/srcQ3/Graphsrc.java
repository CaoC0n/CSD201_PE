...
/*
    ...
    /*
     void f1() - Perform breadth-first traversal (to the file f1 .txt) from the vertex i = 3 (the vertex
    D) but display 5 vertices from the 3nd vertex to the 7th vertex only. Hint: copy depth(...)
    to depth2(...) and modify the latter one. Content of the output file f1.tx( must be:
    D A G B C E H I F
    G B C E H
    */
...
//=================================================================
    ...
    vertices selected into the set Å S with their labels in (2), line 3 contains vertices im shortest
    path (2). (Note that in the weiphted matrix, the value 99 Â¡s considered as infinity). Output
    in the file f2.txt must be the following:
    C E D F
    D,I9 F,24 G,29
    A B C E D G
    // co 1 cau tuong tu D$9 F$14 G$19
                         B C E D G
                         C E D F
...
//        System.out.println("The path is:");
    ...
    /*void f1Q â€” Perform depth first traversal (to the file f1.xt) from the vertex i=0 (the vertex A)
    ...
    void depth2t(boolean[] visited, int k, RandomAccessFile f) throws Exception {
        if (deg(k) > 1) {
            fvisit(k, f);
        }
        visited[k] = true;
    ...
    void depth2t(int k, RandomAccessFile f) throws Exception {
        boolean[] visited = new boolean[20];
        int i;
        for (i = 0; i < n; i++) {
            visited[i] = false;
...
//=================================================================
    /*
    void f2() â€” Supposed the given graph has Euler's cycle. Apply the pseudocode in the
    ...
    void fvisit2t(int i, RandomAccessFile f) throws Exception {
        f.writeBytes(" " + v[i] + "(" + deg(i) + ")");
    }

    void depth2tt(boolean[] visited, int k, RandomAccessFile f) throws Exception {
        fvisit2(k, f);
        visited[k] = true;
        for (int i = 0; i < n; i++) {
            if (!visited[i] && a[k][i] > 0) {
    ...
    void depth2tt(int k, RandomAccessFile f) throws Exception {
        boolean[] visited = new boolean[20];
        int i;
        for (i = 0; i < n; i++) {
            visited[i] = false;
...
//===========================================================================
    ...
    /*
    void f2() - Apply the Dijkstraâ€™s shortest path algorithm to find the shortest path from the vertex 0
...
//===========================================================================
     ...
     */
 /*
    void f3() â€” Supposed the given graph has Euler's cycle. Apply the pseudocode in the Graph.java
   ...
   // int count = 1;
    ...
    void depth2ttt(boolean[] visited, int k, RandomAccessFile f) throws Exception {
        if (count >= 2 && count <= 6) {
            fvisit(k, f);
        }
        visited[k] = true;
    ...
    void depth2ttt(int k, RandomAccessFile f) throws Exception {
        boolean[] visited = new boolean[20];
        int i;
        for (i = 0; i < n; i++) {
            visited[i] = false;
...
//=================================================================
    /*
    void f2() â€” Apply the Dijkstraâ€™s shortest path algorithm to find the shortest path from the
    ...
    void dijkstra2t(int u, int z, RandomAccessFile f) throws IOException, Exception {
    ...
    /*
    void f1()â€” Perform depth first traversal (to the file f1.txt) from the vertex i = 2 (the vertex C)
    but display first 6 vertices only. Hint: copy Ä‘epth(...) to depth2(...) and modify the latter
    one. Content of the output file f1.txt must be:
    C H D A E F I G B
    C H D A E F
     */
    void f1ttttt() throws Exception {
        loadData(1);
        String fname = "f1.txt";
        File g123 = new File(fname);
        if (g123.exists()) {
   ...
   // int count = 0;
    ...
    void depth2tttt(boolean[] visited, int k, RandomAccessFile f) throws Exception {
        if (count < 6) {
            fvisit(k, f);
            count++;
        }
    ...
    void depth2tttt(int k, RandomAccessFile f) throws Exception {
        boolean[] visited = new boolean[20];
        int i;
        for (i = 0; i < n; i++) {
            visited[i] = false;
...
//=================================================================
    ...
    infinity). Output Ã­n the file f2.txt must be the following:
    A  B  C  E  D  G
    D-19 F-24 G-29
     */
    void f2ttttt() throws Exception {
        loadData(12);
        String fname = "f2.txt";
        File g123 = new File(fname);
        if (g123.exists()) {
...
//
    ...
    void dijkstratt(ArrayList<Integer> path, ArrayList<Integer> setS, int fro, int to, RandomAccessFile f) throws Exception {
        int INF = 99;
        boolean[] S = new boolean[n];
        int[] d = new int[n];
        int[] p = new int[n];
...
//        dijkstra2(0, 6, f);
    ...
    void dijkstrat(int fro, int to, RandomAccessFile f) throws Exception {
...
//        f.writeBytes(v[ss.get(fro)]+"->" +v[ss.get(to)]+ d[ss.get(ss.size()-1)]);
    ...
    void dijkstra2tt(int fro, int to, RandomAccessFile f) throws Exception {
        int INF = 999; // infinity value
        boolean[] S = new boolean[n]; // kiem tra xem da duyet dinh chua
        int[] d = new int[n]; // luu gia tri duong di ngan nhat tai dinh do
        int[] p = new int[n]; // luu gia tri dinh gan no nhat
...
//  A B C E D G 29
//  (A,0)(B,10)(C,12)
    void dijkstratt(int fro, int to, RandomAccessFile f) throws Exception {
        int i, j, k, t, INF;
        INF = 999;
        boolean[] S = new boolean[n];
        int[] d = new int[n];
    ...
    void dijkstra2ttt(int fro, int to, RandomAccessFile f) throws Exception {
        int i, j, k, t, INF;
        INF = 999;
        boolean[] S = new boolean[n];
        int[] d = new int[n];
