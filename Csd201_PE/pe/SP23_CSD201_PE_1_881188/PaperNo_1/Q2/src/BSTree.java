...
//    }
    ...
    void insert(String xName, double xPrice, int xColor) {
        ...
        insert2(new Laptop(xName, xPrice, xColor));
        
    }
    
    void insert2(Laptop x) {
        if (isEmpty()) {
            root = new Node(x);
            return;
        }
        ...
        while (child != null) {
            if (child.getInfo().name == x.name) {
                System.out.println("The key " + x + "already exists, no insertion");
                return;
            }
            father = child;
            if (x.name.compareTo(child.getInfo().name) < 0) {
                child = child.left;
            } else {
                child = child.right;
            }
        ...
        if (x.name.compareTo(father.getInfo().name) < 0) {
    ...
    Node insert3(Node p, int x) {
        ...
        if (x < p.getInfo().price) {
            p.left = insert3(p.left, x);

        } else {
            p.right = insert3(p.right, x);
        }
        return (p);
    }
